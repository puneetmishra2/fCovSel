function [vsel,crbvar,beta] = fcovsel(X,Y,lvs)
...
for i = 1:lvs  % loop for selecting variables
    [~,vsel(1,i)] = max( sum( (X'*Y).^2 ,2) ); % % Variable of maximum covariance with Y
...
for i =1:size(Y,2) % The X-regression coefficients and intercepts for responses
    ...
    beta{i}  = [mean(Yorig(:,i)) - mean(Xorig(:,vsel))*beta{i}; beta{i}];
end
end
